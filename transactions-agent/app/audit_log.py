"""Structured token-lifecycle audit trail.

Every OAuth/OIDC token created, refreshed, validated, or cached anywhere in the system
gets one JSON line appended to a shared file, modeled as a hop between two named actors
(origin -> destination) so a later viewer can draw an actual sequence/graph of exchanges
— not just a flat list of log lines. Correlated by the per-prompt transaction_id (and the
per-connection session_id) via contextvars, so call sites deep in auth_manager.py/
gateway.py don't need their signatures changed to thread an ID through.

Both transactions-agent and savings-goals-agent append to the SAME file
(.demo-logs/token-audit.jsonl at the repo root) — single JSON-line writes from separate
...
# in-memory here, but a DIFFERENT process (e.g. savings-goals-agent validating an
# inbound call FROM the Coordinator) needs to resolve the CALLER's identity too — which
# it never registered itself, since it's a separate Python interpreter with no shared
# memory. The shared file is the only thing both processes can see.
